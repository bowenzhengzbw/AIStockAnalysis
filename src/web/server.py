...
class AnalysisHandler(BaseHTTPRequestHandler):
    ...
    def _dispatch(self, *, send_body: bool) -> None:
        ...
        if path in {"", "/"}:
            index_html = (
                ...
                "<li><a href=\"/portfolio/report\">/portfolio/report</a> — 组合策略巡航 JSON</li>"
                "<li><a href=\"/portfolio/report?format=html\">/portfolio/report?format=html</a> — 组合策略巡航 HTML 预览</li>"
                "</ul></body></html>"
            )
        ...
        if path == "/risk/report":
            ...
            return
        if path == "/portfolio/report":
            response_format = query.get("format", ["json"])[0].lower()
            self._handle_portfolio_report(
                response_format=response_format, send_body=send_body
            )
            return
        if send_body:
    ...
    def _handle_company_report(
        ...
        try:
            ...
            LOGGER.exception("Failed to build company report: %s", exc)
            self._write_json({"error": "internal_error", "detail": str(exc)}, status=500)
            return
        if response_format == "html":
            self._write_html(report.to_html(), send_body=send_body)
    ...
    def _handle_portfolio_report(
        ...
        try:
            ...
            LOGGER.exception("Failed to build portfolio report: %s", exc)
            self._write_json({"error": "internal_error", "detail": str(exc)}, status=500)
            return
        if response_format == "html":
            self._write_html(report.to_html(), send_body=send_body)
...
def create_server(host: str = "127.0.0.1", port: int = 8000) -> ThreadingHTTPServer:
    server: ThreadingHTTPServer = ThreadingHTTPServer((host, port), AnalysisHandler)
    return server
