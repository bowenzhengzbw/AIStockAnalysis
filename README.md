...
- **实时代码联想**：输入框支持自动补全，下拉菜单仅展示能够成功抓取数据的股票或 ETF 代码，并对网络代理进行了自动降级处理。
...
- 如果自动完成功能提示网络异常，通常是当前环境无法连接 Yahoo Finance。系统会优先尝试直连，再回退到环境变量指定的代理（如 `HTTPS_PROXY`），仍失败时请确认代理/防火墙设置并确保其允许访问 `query1.finance.yahoo.com`。
